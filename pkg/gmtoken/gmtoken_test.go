...
import (
	...
	"github.com/hxx258456/pyramidel-chain-baas/pkg/tokenrandom"
...
func TestBuildTokenWithGM(t *testing.T) {
	...
	// 从pem文件读取公钥
	certFile, err := ioutil.ReadFile("testdata/sm2_pub_key.pem")
	if err != nil {
		t.Fatal(err)
	}
	// 解码 PEM 格式的证书
	...
	if err != nil {
		fmt.Printf("Failed to parse cert file: %v\n", err)
		os.Exit(1)
	}
...
func TestBuildTokenWithSm2Sm3(t *testing.T) {
	...
	token1, err := CheckTokenWithECC(token.TokenStr, block.Bytes)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildTokenWithHMACSM3(t *testing.T) {
	keyBytes, _ := tokenrandom.GenerateRandomBytes(64)
