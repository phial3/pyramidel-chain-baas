...
func CreateStdPayloads(iss string, sub string, aud string, jti string, expSeconds uint64) map[string]string {
	now := time.Now()
	strNow := now.Format(constants.TOKEN_TIME_FORMAT_SIMPLE)
	...
	payloads["jti"] = jti
	// 过期时间
	payloads["exp"] = now.Add(time.Second * time.Duration(expSeconds)).Format(constants.TOKEN_TIME_FORMAT_SIMPLE)
	// 生效时间
	payloads["nbf"] = strNow
	// 签发时间
	payloads["iat"] = strNow
...
func BuildTokenWithECC(token *Token, exp time.Time, priKeyPem []byte) error {
	...
	// 重置凭证过期时间
	if !exp.IsZero() {
		payloads["exp"] = exp.Format(constants.TOKEN_TIME_FORMAT_SIMPLE)
	}
	// 将token的有效负载转为json
...
func CheckTokenWithECC(tokenStr string, pubKeyPem []byte) (*Token, error) {
	...
	if expVal != "" {
		now := time.Now()
		exp, err := time.ParseInLocation(constants.TOKEN_TIME_FORMAT_SIMPLE, expVal, time.Local)
		if err != nil {
			return nil, fmt.Errorf("[-5]token过期时间反序列化失败: %s", err)
		}
		if now.After(exp) {
			return nil, fmt.Errorf("[-1]token过期,过期时间: %s, 检查时间: %s", expVal, now.Format(constants.TOKEN_TIME_FORMAT_SIMPLE))
		}
	}
